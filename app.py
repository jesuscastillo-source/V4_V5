...
def _listar_librerias_faltantes(ruta_binario):
    """Corre 'ldd' sobre el binario para ver TODA la lista de librerías que
    necesita, y devuelve solo las que están marcadas como 'not found' — así,
    si al servidor le falta más de una, las vemos todas de una vez en el
    primer intento, en vez de una por una en cada redeploy."""
    try:
        resultado = subprocess.run(["ldd", ruta_binario], capture_output=True, timeout=30, text=True)
        faltantes = []
        for linea in resultado.stdout.splitlines():
            if "not found" in linea:
...
def _convertir_lote_soffice(docx_por_nombre: dict, tmpdir_base=None) -> tuple:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if not resultados:
            ...
            if "error while loading shared libraries" in stderr or "cannot open shared object" in stderr:
                todas_las_que_faltan = _listar_librerias_faltantes(ruta_soffice)
                if todas_las_que_faltan:
                    diagnostico += f" | TODAS las librerías faltantes detectadas: {', '.join(todas_las_que_faltan)}"
